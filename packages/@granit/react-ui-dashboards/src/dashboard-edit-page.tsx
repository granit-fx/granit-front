...
/**
 * Rich per-dashboard composer. End-to-end demo of the framework primitives:
 *
 * - **Load** — `useDashboardDetail(id)` → `dashboardDetailToDefinition`
 *   bridges the persisted-instance shape (configJson + Guid ids) into
 *   the editor's `DashboardDefinition`.
 * - **Compose** — three catalogs (framework + analytics + map) merged
 *   once via `composeCatalogs` / `composeWidgetConfigFormRegistries`.
 * - **Edit** — `<EditableDashboard>` (drag-reorder + drag-resize),
 *   `<WidgetPalette>` rendered inside an "+ Add widget" dialog,
 *   `<WidgetConfigDrawer>` rendered inside a per-widget Edit dialog
 *   that opens on cell click.
 * - **Save** — diff local vs server via `diffDashboardWidgets`;
 *   dispatch the resulting sequence (removed → added → updated)
 *   through the per-widget CRUD hooks. Metadata edits piggyback on a
 *   separate `useUpdateDashboardMetadata` call when name / layout
 *   dirty.
 *
 * Adjust-state-during-render pattern: when the server payload changes
 * (post-save invalidation), the local working copy resets to the new
 * canonical detail. No useEffect setState cascade.
 */
export function DashboardEditPage() {
  ...
  // bubbled clicks from `<EditableDashboard>`'s cells. Hooks must run
  // before the loading early-return below.
  ...
  return (
    <div data-slot="dashboard-edit-page" className="space-y-4">
