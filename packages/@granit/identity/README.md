...
| `IdentityUserCacheSyncAllResponse`         | type  | `{ syncedCount }` from a full sync                          |
| `IdentityUserCacheSyncStaleResponse`       | type  | `{ refreshedCount }` from a stale sync                      |
...
- **Roles and groups are addressed by name/id, not by branded type.** `assignRole` /
  `removeRole` take the role _name_; `addUserToGroup` / `removeUserFromGroup` take the
  group _id_ as a plain string. The `IdentityRoleId` / `IdentityGroupId` brands type the
  _response_ records, not these mutation arguments.
