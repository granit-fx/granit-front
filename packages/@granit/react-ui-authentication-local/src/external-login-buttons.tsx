...
import { HttpError } from '@granit/react-authentication-local';
 ...
 *
 * Each button calls `POST /external-logins/challenge/{name}` through the headless
 * `useChallengeExternalLogin` hook. The SDK rethrows the framework's error
 * contract as an {@link HttpError}: `400` (provider not configured) or `500`
 * (configured but its auth handler is not registered on the host). Those surface
 * as scoped toasts here.
 *
 * SEAM. A live backend answers the challenge with a `302` to the identity
 * provider; the browser navigation is what actually starts the OAuth dance.
 */
